modify_grapes_data <- function(max_ncfiles, min_ncfiles, wind_ncfiles, pr_ncfiles, avt_ncfiles, solardata, station_number) {
  library(ncdf4)
  
  # Select the right column of solardata per location
  soldata <- solardata[, station_number + 6]
  soldata <- soldata / 1000  # Conversion from kJ to MJ per m²
  
  # Explanatory print statement
  print("The csv's files are created for each location, it takes about 10 minutes to create 1 csv. The total runtime for the function modify_grapes_data is around 50-55 minutes")
  ...
  print("Precipitation:")
  # Pre-allocate precipitation to store the combined wind data
  prdata <- numeric(0)
  ...
  for (file in avt_ncfiles) {
    ...
    vp <- A * exp((B * avtdata) / (avtdata + C)) / 10
  ...
  # Calculate Julian day separately
  julian_day <- as.integer(dates - as.Date("1850-01-01"))
  julian_day <- julian_day + 2396758  # Adjusting to start from correct Julian day for Jan 1, 1850
  
  # Create the csv data frame, combining all data
  csv_data <- data.frame(
    ...
    Month = as.integer(format(dates[1:valid_length], "%m")),
    DayofMonth = as.integer(format(dates[1:valid_length], "%d")),
  ...
  file_name <- paste("grapes_", station_number, ".csv", sep = "")
  
  # Generate the full file path including the subfolder
  file_path <- file.path(folder_path, file_name)
  
  # Write the data frame to a CABO file
  write.table(csv_data, file_path, sep = ";", row.names = FALSE, col.names = FALSE, quote = FALSE)
